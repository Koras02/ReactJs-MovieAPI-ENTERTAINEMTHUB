...
export default function ContentBottomPanel({
  ...
  const getTvDetailLink = (id: number): string => {
    return `https://api.themoviedb.org/3/tv/${id}?api_key=7db8b1ffbba88aaa67068565d84fe99f&language=ko`;
  };

  const getYoutubeLink = (key: string): string => `https://www.youtube.com/watch?=${key}`;
      ...
      {hasClickedContent && tvDetail && (
        <>
          ...
          <Styled.Text>
           ...
           <Styled.Text>
              <Styled.GrayText>국가:</Styled.GrayText>   
